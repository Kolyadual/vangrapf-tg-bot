...
def get_youtube_stream(url: str) -> str:
    try:
        resp = requests.post(f"{API_URL}/stream-url", json={"url": url}, headers={"Content-Type": "application/json"}, timeout=60)
        data = resp.json()
...
def get_direct_link(url: str) -> str:
    try:
        resp = requests.get(f"{API_URL}/dl", params={"url": url}, allow_redirects=False, timeout=60)
...
def download_video_file(url: str) -> bytes:
    try:
        resp = requests.post(f"{API_URL}/download", json={"url": url}, headers={"Content-Type": "application/json"}, timeout=300)
        ...
        if platform == "youtube":
            ...
            if video_data and len(video_data) < 50 * 1024 * 1024:
                ...
                with open("/tmp/video.mp4", "wb") as f:
                    f.write(video_data)
                await update.message.reply_video(video=open("/tmp/video.mp4", "rb"), caption="✅ Готово!")
                await status_msg.delete()
            else:
                await status_msg.edit_text("📡 Получаю ссылку на стрим...")
                stream_url = get_youtube_stream(url)
                if stream_url:
                    await status_msg.edit_text(f"✅ [Открыть видео]({stream_url})", parse_mode="Markdown", disable_web_page_preview=False)
                else:
            ...
            if direct_link:
                await status_msg.edit_text(f"✅ [Скачать файл]({direct_link})", parse_mode="Markdown", disable_web_page_preview=True)
            else:
